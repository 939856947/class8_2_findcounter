...
int main()
{
	cv::Mat srcMat = imread("D:/22.jpg", 0);
	cv::Mat src;
	cv::Mat src1;
	int i, j, t, k = 0, kk = 0, k1 = 0;
	cv::Mat dst = imread("D:/22.jpg");
	int height = dst.rows;
	int width = dst.cols;
	...
	for (i = 0; i < contours.size(); i++) {
		...
		for (j = 0; j < contours[i].size(); j++) {
			if (contours[i][j].x>width1&&contours[i][j].x<width2&&contours[i][j].y>height1&&contours[i][j].y < height2)
				k++;
		}
		...
		if (k > 150 && k < 500) {
			...
			rbox.points(vtx);
			for (int i = 0; i < 4; i++) {
				cv::line(dst, vtx[i], vtx[i < 3 ? i + 1 : 0], cv::Scalar(255, 20, 147), 2, CV_AA);
			}//画外接四边形
		}
	}
	imshow("src", src);
